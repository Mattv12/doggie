...
class MotorModule(Module):
    ...
    def tick(self, context) -> None:
        ...
        web_head = context.get("web_head_action")
        if isinstance(web_head, str):
            context.set("web_head_action", None)
            self._apply_web_head(web_head, context)
        web_body = context.get("web_body_action")
        if isinstance(web_body, str):
            context.set("web_body_action", None)
            if web_body == "stop":
                self._hardware_stop()
    ...
    def _apply_web_head(self, command: str, context) -> None:
        """Apply one small, bounded manual head adjustment from the LAN controller."""
        if self.dog is None or not hasattr(self.dog, "head_move"):
            return
        settings = (context.get("settings") or {}).get("web_controller", {})
        step = _safe_int(settings.get("head_step_deg", 12), 12)
        speed = _safe_int(settings.get("head_speed", 70), 70)
        coords = {"left": (step, 0), "right": (-step, 0), "up": (0, step), "down": (0, -step), "center": (0, 0)}
        if command not in coords:
            return
        yaw, pitch = coords[command]
        try:
            self.dog.head_move([[yaw, pitch, 0]], speed=speed)
            if hasattr(self.dog, "wait_head_done"):
                self.dog.wait_head_done()
        except Exception as exc:  # noqa: BLE001
            logger.warning("Manual head movement failed: %s", exc)
