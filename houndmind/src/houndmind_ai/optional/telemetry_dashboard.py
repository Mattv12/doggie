...
class TelemetryDashboardModule(Module):
    ...
    def start(self, context) -> None:
        ...
        self._controller_enqueue = context.get("web_controller_enqueue")
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if self.path == "/":
                    ...
                    # ``{{HOST}}`` keeps the page portable: the same URL works
                    # on a laptop or phone without hard-coding the Pi's DHCP IP.
                    host = self.headers.get("Host", "").split(":", 1)[0]
                    camera_path = camera_path.replace("{{HOST}}", host)
                ...
                if self.path == "/controller":
                    from houndmind_ai.optional.web_controller import _PAGE
                    host = self.headers.get("Host", "").split(":", 1)[0]
                    html = _PAGE.replace("{{HOST}}", host).encode("utf-8")
                    self.send_response(200)
            ...
            def do_POST(self):
                if urlparse(self.path).path not in ("/command", "/controller/command"):
                    self._send_json({"error": "Not found"}, status=404)
                    return
                handler = getattr(module, "_controller_enqueue", None)
                if not callable(handler):
                    self._send_json({"error": "controller unavailable"}, status=503)
                    return
                try:
                ...
                self._send_json({"status": "queued", "command": message} if ok else {"error": message}, 200 if ok else 400)
