...
class LedManagerModule(Module):
    ...
    def tick(self, context) -> None:
        ...
        priority = settings.get(
            "priority", ["safety", "health", "navigation", "attention", "emotion"]
        )
        ...
        self._apply_led(context, source, mode, request)
    ...
    def _apply_led(self, context, source: str, mode: str, request: dict) -> None:
        settings = (context.get("settings") or {}).get("led", {})
        ...
        if source == "safety":
            ...
            color = str(request.get("color", settings.get("default_color", "blue")))
            mode_name = settings.get("health_mode", "breath")
        else:
            # navigation modes
            if mode == "patrol":
