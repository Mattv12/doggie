...
| `deploy/pidog-gpt.service` | systemd unit (runs as root, `LOGNAME=matt`, Restart=always) |
| `deploy/dog-deploy` | pull + syntax check + service restart, run via `ssh matt@10.0.0.55 dog-deploy` |

`secret.py` (OpenAI API key) is intentionally untracked — create it on the Pi
by hand: `OPENAI_API_KEY = "sk-..."`.
