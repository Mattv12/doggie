...
class VoiceModule(Module):
    ...
    def start(self, context) -> None:
        ...
        self.available = True
        context.set("voice", {"status": "ready", "last_heard": None, "last_action": None})
        ...
        if self._pending:
            for item in list(self._pending):
                if "action" in item:
                    ...
                    text = str(item["text"])
                    context.set("voice", {"status": "heard", "last_heard": text, "last_action": None})
                    logger.info("Voice heard: %s", text)
                    normalized = self._prepare_command(text, settings)
                    if normalized is None:
                        logger.info("Voice ignored: wake word required")
    ...
    def _prepare_command(self, text: str, settings: dict) -> str | None:
        normalized = self._normalize(text)
        wake_words = [self._normalize(str(word)) for word in settings.get("wake_words", [])]
        matched = False
        for wake_word in wake_words:
            if wake_word and (normalized == wake_word or normalized.startswith(wake_word + " ")):
                normalized = normalized[len(wake_word):].strip()
                matched = True
                break
        if settings.get("require_wake_word", False) and not matched:
            return None
        return normalized
    ...
    def _apply_action(self, action: str, context) -> None:
        ...
        context.set("voice", {**voice, "status": "command", "last_action": action})
    ...
    def _stt_loop(self, context) -> None:
        ...
        if backend in ("auto", "speech_recognition", "speech-recognition"):
            try:
                ...
                r.dynamic_energy_threshold = bool(stt_cfg.get("dynamic_energy_threshold", True))
                if stt_cfg.get("energy_threshold") is not None:
                    r.energy_threshold = int(stt_cfg["energy_threshold"])
                r.pause_threshold = float(stt_cfg.get("pause_threshold_s", 0.5))
                r.non_speaking_duration = float(stt_cfg.get("non_speaking_duration_s", 0.3))
                with mic as source:
                    r.adjust_for_ambient_noise(source, duration=float(stt_cfg.get("ambient_calibration_s", 1.0)))
                context.set("voice_stt_status", {"status": "listening", "backend": "speech_recognition", "device_index": device_index})
                logger.info("SpeechRecognition STT started (device=%s, energy=%s)", device_index, r.energy_threshold)
                while not self._stt_stop.is_set():
                    try:
                        with mic as source:
                            audio = r.listen(
                                source,
                                timeout=float(stt_cfg.get("listen_timeout_s", 3.0)),
                                phrase_time_limit=float(stt_cfg.get("phrase_time_limit_s", 3.0)),
                            )
                        try:
                ...
                logger.exception("SpeechRecognition STT failed to initialize")
                context.set("voice_stt_status", {"status": "error", "backend": "speech_recognition"})
