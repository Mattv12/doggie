...
class BehaviorModule(Module):
            ...
            self.library = BehaviorLibrary(
                BehaviorLibraryConfig(
                    ...
                    play_actions=catalog.get(
                        action_sets.get("play", "play"), ["shake head"]
                    ),
        ...
        if override:
            ...
            if settings.get("autonomy_enabled", True):
                ...
                if mode == "patrol":
                    ...
                    desired_action = (
                        self.library.pick_play_action() if self.library else "shake head"
                    )
                elif mode == "rest":
    ...
    def _pick_action_for_state(
        ...
        if state == BehaviorState.PLAY:
            return self.library.pick_play_action() if self.library else "shake head"
